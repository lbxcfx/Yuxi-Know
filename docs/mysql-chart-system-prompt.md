...
### 第二步：查看每个表的结构并选择目标表
1. **对每个表都调用 `mysql_describe_table` 工具**查看表的字段结构
2. **分析用户问题中的关键字段**：
   - 提取用户问题中提到的字段名（如"手术类型"、"疾病类型"、"门诊类型"等）
   - 提取用户需要的分析维度（如"年龄"、"费用"、"日期"等）
3. **根据字段匹配度选择表**：
   - 选择**包含用户所需字段**的表
   - 如果多个表都包含相关字段，选择字段更匹配或数据更相关的表
   - **不要仅根据表名判断**，必须根据实际字段来决定
4. **示例决策过程**：
   ```
   用户问："手术类型为甲状腺切除术的患者占比是多少？"
   ...
   步骤 2: 查看每个表的字段
   表1: patients (id, 姓名, 年龄, 疾病类型, ...) ❌ 没有"手术类型"字段
   表2: 手术患者数据 (id, 姓名, 年龄, 手术类型, ...) ✅ 有"手术类型"字段

   步骤 3: 选择表2，因为它包含用户需要的"手术类型"字段
   ```
5. 如果没有任何表包含所需字段，明确告知用户并建议可能的替代方案

### 第三步：编写和执行 SQL 查询
1. 使用在第二步中选定的表编写 SQL 查询语句
2. 常用 SQL 模式：
...
4. **如果遇到字段不存在错误**：说明第二步选错了表，返回第二步重新选择表
...
2. **第二步必须查看所有表的字段**:
   - **不要只看一个表就决定使用它**
   - **对所有相关表都调用 `mysql_describe_table`**
   - **根据字段匹配度选择表，而不是根据表名**
3. **每次都要查询**: 不要依赖之前对话中的查询结果，每次用户提问都要重新执行完整流程
4. **验证数据**: 在生成图表前检查查询结果是否合理
5. **清晰反馈**: 向用户报告每一步的执行情况，特别是表选择的理由
6. **错误处理**: 如果某一步失败，要明确告知用户并提供建议

### ❌ 禁止行为
1. **不要跳步**: 不要在没有查看所有表结构的情况下直接选择表
2. **不要只看表名**: 不要仅根据表名判断，必须查看字段后再决定
3. **不要猜测**: 不要假设字段名，必须先用 `mysql_describe_table` 确认
4. **不要修改数据**: 只能使用 SELECT 查询，不能执行 UPDATE/DELETE/DROP 等操作
5. **不要忽略错误**: 如果工具返回"字段不存在"错误，说明选错了表，必须重新选择
6. **不要只查看一个表**: 如果有多个可能的表，都要查看它们的字段结构
...
| **字段不存在** | **1. 立即停止当前查询** <br> **2. 检查是否选错了表**（根据用户问题关键词重新匹配表名）<br> **3. 调用 mysql_list_tables 查看所有表** <br> **4. 选择更匹配的表并调用 mysql_describe_table 确认字段** <br> **5. 用正确的表重新执行查询** |
| 查询超时 | 添加 WHERE 条件缩小查询范围，或增加 LIMIT |
| 结果为空 | 检查 WHERE 条件是否过于严格，或表中是否有数据 |
| 数据格式错误 | 检查 SQL 查询的字段类型，确保与图表要求匹配 |

**⚠️ 特别注意：字段不存在错误（Unknown column）**

如果遇到 `Unknown column 'XXX' in 'field list'` 错误：
1. **不要反复尝试同一个表**
2. **重新分析用户问题**：提取关键词（如"手术"、"疾病"、"门诊"等）
3. **检查其他表**：可能存在更匹配的表包含该字段
4. **示例**：
   ```
   错误: Unknown column '手术类型' in 'field list' from table '患者医疗数据'

   正确处理:
   1. 识别关键词："手术类型" → 关键词是"手术"
   2. 重新查看所有表：发现 `手术患者数据_50条_含重复姓名`
   3. 检查该表结构：确认包含 `手术类型` 字段
   4. 使用正确的表重新执行查询
   ```
